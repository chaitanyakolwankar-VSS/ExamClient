...
export default function Gazette() {
  ...
  // Toggles
  const [roundNumber, setRoundNumber] = useState(true);
  const [noRleForFail, setNoRleForFail] = useState(true);
  ...
  const [subjectsPerRow, setSubjectsPerRow] = useState("7");
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      alert("Failed to download Gazette.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDownloadExcel = async () => {
    ...
    try {
      ...
      alert("Failed to download Gazette Excel.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="p-6 space-y-6">
      <ComponentCard title="Generate Gazette Report">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <Select
              ...
              onChange={(value) => setSelectedPattern(value)}
        ...
        <div className="mt-6 flex justify-end gap-3 items-center">
          ...
          <Button onClick={handleDownloadPdf} disabled={loading} className="flex items-center gap-2">
            <Download size={18} />
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={roundNumber} onChange={(e) => setRoundNumber(e.target.checked)} className="rounded text-blue-600" />
                <span className="text-sm text-gray-700 dark:text-gray-300">Round Number</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={noRleForFail} onChange={(e) => setNoRleForFail(e.target.checked)} className="rounded text-blue-600" />
                <span className="text-sm text-gray-700 dark:text-gray-300">No RLE For Fail</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={cgpiForFail} onChange={(e) => setCgpiForFail(e.target.checked)} className="rounded text-blue-600" />
                <span className="text-sm text-gray-700 dark:text-gray-300">CGPI For Fail</span>
              </label>
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={sgpiForFail} onChange={(e) => setSgpiForFail(e.target.checked)} className="rounded text-blue-600" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <select 
                  ...
                  className="w-full max-w-[200px] px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-md bg-white dark:bg-gray-800"
                >
